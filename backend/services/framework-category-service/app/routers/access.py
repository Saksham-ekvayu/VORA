...
def _format_access_record(
    ...
    result = {
        "id": str(record.id),
        "expert": data_format.format_user_ref(users_by_id.get(str(record.expertId)), record.expertId),
        "frameworkCategory": None,
        "status": record.status,
    }
    if category:
        result["frameworkCategory"] = {
...
async def _batch_format(records: list[FrameworkAccess]) -> list[dict]:
    ...
    return [_format_access_record(r, users_by_id, categories_by_id) for r in records]
...
def _build_access_list_query(stmt, status: str, expert_id: str | None, framework_code: str | None):
    ...
    if expert_id:
        stmt = stmt.where(FrameworkAccess.expertId == expert_id)
    if framework_code:
        stmt = stmt.where(FrameworkAccess.frameworkCode == framework_code.lower())
    return stmt
...
async def request_framework_access(
    ...
    logger.info(
        f"[REQUEST-ACCESS] Request started | user_id={ctx.user.id} | category_id={framework_category_id}"
    )
    user = ctx.user
    ...
    async with session_scope() as session:
        ...
        if existing_access:
            if existing_access.status == "approved":
                logger.warning(
                    f"[REQUEST-ACCESS] Already has access | user_id={user.id} | category_id={framework_category_id}"
                )
                return error(
                    msg.MESSAGES.get(
            ...
            if existing_access.status == "pending":
                logger.warning(
                    f"[REQUEST-ACCESS] Already has pending | user_id={user.id} | category_id={framework_category_id}"
                )
                return error(
                    msg.MESSAGES.get(
            ...
            if existing_access.status in ("rejected", "revoked"):
                ...
                logger.info(
                    f"[REQUEST-ACCESS] Resubmitted | user_id={user.id} | category_id={framework_category_id} | old_status={existing_access.status}"
                )
        ...
        if not existing_access:
            ...
            logger.info(
                f"[REQUEST-ACCESS] New request created | user_id={user.id} | category_id={framework_category_id}"
            )
...
async def _create_or_update_access_record(session, expert_id: str, category, auth_user_id: str, now):
    ...
    if existing:
        ...
        existing.approval = ApprovalInfo(approvedBy=auth_user_id, approvedAt=now).model_dump(mode="json")
        ...
        access_record = FrameworkAccess(
            ...
            status="approved",
            requestedBy="admin",
            approval=ApprovalInfo(approvedBy=auth_user_id, approvedAt=now).model_dump(mode="json"),
        )
...
async def assign_framework_access(
    ...
    async with session_scope() as session:
        ...
        _, framework_categories, error_msg = await _validate_expert_and_categories(
...
async def approve_framework_access(
    ...
    logger.info(
        f"[APPROVE-ACCESS] Approved | access_id={id} | expert_id={record.expertId} | by={auth.user.id}"
    )
...
async def reject_framework_access(
    ...
    logger.info(
        f"[REJECT-ACCESS] Rejected | access_id={id} | expert_id={record.expertId} | by={auth.user.id}"
    )
...
async def revoke_framework_access(
    ...
    logger.info(
        f"[REVOKE-ACCESS] Request started | user_id={auth.user.id} | expert_id={expert_id} | category_id={framework_category_id}"
    )
    ...
    async with session_scope() as session:
        ...
        record = (
            await session.execute(
                select(FrameworkAccess).where(
                    FrameworkAccess.expertId == expert_id,
                    FrameworkAccess.frameworkCategoryId == framework_category_id,
                )
            )
        ).scalar_one_or_none()
        if not record:
            logger.warning(
                f"[REVOKE-ACCESS] Record not found | expert_id={expert_id} | category_id={framework_category_id}"
            )
    ...
    logger.info(
        f"[REVOKE-ACCESS] Revoked | expert_id={expert_id} | category_id={framework_category_id} | by={auth.user.id}"
    )
