        ...
        try:
            async with session_scope() as session:
                ...
                if fw:
                    versions = list(fw.fileVersions or [])
                    idx, fv = _find_file_version(versions, file_id)
                    if idx is not None and fv is not None:
                        ai = dict(fv.get("aiExtraction") or {})
                        ai.update(
                            {
                                "status": "failed",
    ...
    try:
        ...
        async with session_scope() as session:
            ...
            if doc_idx is None:
                return
            doc = documents[doc_idx]
            extraction_id = doc.get("aiExtraction")
...
def _extract_controls_from_ai(ai: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(ai, list):
        # DocumentExtraction stores controls as list of ExtractionControls
        for item in ai:
            if isinstance(item, dict) and "controls_data" in item:
                return item.get("controls_data") or []
        return ai if all(isinstance(x, dict) and "controls" in x for x in ai) else []
    if isinstance(ai, dict):
        controls = ai.get("controls")
        ...
        if isinstance(controls, list):
            ...
            return controls
        return ai.get("controls_data") or []
    return []
    ...
    try:
        ...
        async with session_scope() as session:
            ...
            for doc in documents:
                ...
                if status not in ("extracted", "completed") and not (
                    isinstance(ai_payload, list) and ai_payload
                ):
                    # still try to pull controls if present
                    pass
