...
const transformChartData = (data) => {
  ...
  return data.map((item) => {
    // If it's a YYYY-MM-DD date, we format it. Otherwise we keep the month string.
    let displayDate = item.month;
    if (/^\d{4}-\d{2}-\d{2}$/.test(item.month)) {
      displayDate = new Date(item.month).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "2-digit",
      });
    ...
    return {
      date: displayDate,
      fullDate: item.month,
      total: Number(item.uploads) || 0,
    };
  });
};

const UploadTrendChart = ({ data, height = 230 }) => {
  ...
  if (!chartData.length) {
    return (
      <div className="flex h-77.5 items-center justify-center rounded border border-dashed border-border bg-background/75 text-sm text-muted-foreground">
        No framework upload data available.
      </div>
    );
  }
  ...
  const grandTotal = chartData.reduce((a, b) => a + b.total, 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ReferenceLine
          y={chartData.length ? grandTotal / chartData.length : 0}
          stroke={colors.text}
          strokeDasharray="5 5"
          opacity={0.7}
          label={{
